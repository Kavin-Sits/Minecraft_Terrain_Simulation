...
export class GUI implements IGUI {
  ...
  public getMaxHeightForPlayer(chunkManager: ChunkManager, pos: Vec3): number {
    const playerRadius = 0.4;
    const maxHeightCheck = 2.0;
    ...
    for (const corner of corners) {
      ...
      const currentChunk = chunkManager.getPlayerChunk(x, z);
      const blockHeight = currentChunk.getHeightAt(x, z);
      if (blockHeight > maxBlockHeight) {
        maxBlockHeight = blockHeight;
      }
    }
  
    const maxPlayerHeight = maxBlockHeight + maxHeightCheck;
