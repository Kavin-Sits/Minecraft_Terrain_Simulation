...
export class Chunk {
    ...
    getHeightAt(x, z) {
        ...
        /*for (let k = 0; k < 4; k++) {
            const y = this.cubePositionsF32[idx + 1 + k * 4];
            if (y > max_height) {
                max_height = y;
            }
        }*/
    ...
    createWhiteNoise(width, length, rng) {
        return Array.from({ length: width }, () => Array.from({ length }, () => rng.next()));
    }
    bilinear_iterpolation(a, b, t) {
        return a + t * (b - a);
    }
    interpolateNoise(noise, x, y) {
    ...
    scaleNoise(noise, length, width) {
        return Array.from({ length: width }, (_, x) => Array.from({ length }, (_, y) => {
    ...
    generateTerrain(seed, length = 64, width = 64) {
